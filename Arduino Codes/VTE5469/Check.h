...
SoftwareSerial serialRFID(PA3, PA2);
...
void sendCommand(uint8_t com_nub) {
  ...
  while (RFID_cmdnub[com_nub][b] != 0x7E){
    serialRFID.write(RFID_cmdnub[com_nub][b]);
    if (DEBUG) {
      Serial.print(" 0x");
      Serial.print(RFID_cmdnub[com_nub][b], HEX);
    } b++;
  }
  serialRFID.write(0x7E);
  serialRFID.write("\n\r");
  Serial.println();
}
...
void Readcallback(){
  ...
  while (serialRFID.available()){
    delay(2);
    DATA_I[DATA_I_NUB] = serialRFID.read();
    if (DEBUG == 1){
      if (DATA_I[DATA_I_NUB] < 16)Serial.print(" 0x0");
